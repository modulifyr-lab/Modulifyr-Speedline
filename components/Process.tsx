      ...
      <div className="max-w-[1120px] mx-auto px-5 sm:px-8 md:px-12">
        ...
        <Reveal delay={60}>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-0.5">
            {STEPS.map(step => (
              <div key={step.num}
                className="group border border-sl-border px-4 sm:px-[18px] py-6 sm:py-7 text-center transition-colors duration-200 hover:border-sl-orange" style={{ backgroundColor: 'var(--color-bg)' }}>
                <div className="w-[34px] h-[34px] rounded-full border border-sl-border flex items-center justify-center mx-auto mb-4 font-mono text-[10px] text-sl-orange transition-all duration-200 group-hover:bg-sl-orange group-hover:text-sl-white group-hover:border-sl-orange" style={{ backgroundColor: 'var(--color-surface)' }}>
                  {step.num}
                </div>
                <h3 className="font-syne font-bold text-[13px] mb-2" style={{ color: 'var(--color-text)' }}>{step.title}</h3>
                <p className="text-[11px] leading-relaxed" style={{ color: 'var(--color-text-muted)' }}>{step.desc}</p>
              </div>
            ))}
          </div>
        </Reveal>
